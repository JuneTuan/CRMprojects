...
export class PrizeManager {
  ...
  async getPrizes(options: any = {}) {
    ...
    let prizes = JsonStorage.readData<any>(this.FILE_NAME);
    
    if (filters.id) {
      prizes = prizes.filter((prize: any) => prize.id === filters.id);
    }
    if (filters.type) {
      prizes = prizes.filter((prize: any) => prize.type === filters.type);
    }
