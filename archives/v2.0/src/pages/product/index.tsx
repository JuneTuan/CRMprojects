...
export default function ProductPage() {
  ...
  if (showForm) {
    return (
      <View className={`product-page min-h-screen bg-gray-50 ${responsive.padding[breakpoint]}`}>
        ...
        <View className={`bg-white rounded-2xl ${responsive.padding.md} shadow-sm`}>
          <View className="mb-4">
            <Text className={`block ${responsive.text.sm} font-semibold text-gray-800 mb-2`}>产品名称 *</Text>
            <View className={`bg-gray-50 rounded-xl ${responsive.paddingX.md} ${responsive.paddingY.md}`}>
              <Input
          ...
          <View className="mb-4">
            <Text className={`block ${responsive.text.sm} font-semibold text-gray-800 mb-2`}>产品描述</Text>
            <View className={`bg-gray-50 rounded-xl ${responsive.paddingX.md} ${responsive.paddingY.md}`}>
              <Input
                className="w-full bg-transparent"
                placeholder="请输入产品描述"
          ...
          <View className="mb-4">
            <Text className={`block ${responsive.text.sm} font-semibold text-gray-800 mb-2`}>产品价格 *</Text>
            <View className={`bg-gray-50 rounded-xl ${responsive.paddingX.md} ${responsive.paddingY.md}`}>
              <Input
                className="w-full bg-transparent"
                type="digit"
  ...
  return (
    <View className={`product-page min-h-screen bg-gray-50 pb-20 ${responsive.padding[breakpoint]}`}>
      <View className="flex justify-between items-center mb-4">
        <Text className={`${responsive.text.xl} font-bold text-gray-800`}>产品管理</Text>
        <Button
      ...
      <View className={`bg-white rounded-xl ${responsive.paddingX.md} ${responsive.paddingY.md} mb-4 shadow-sm`}>
        <Input
          className={`w-full bg-transparent ${responsive.text.md} outline-none`}
          placeholder="搜索产品名称、描述或价格"
          value={searchKeyword}
      ...
      {loading ? (
        ...
        <View className="flex flex-col items-center justify-center py-16">
          ...
          <Text className="block text-lg font-semibold text-gray-600 mb-2">
            {searchKeyword ? '未找到匹配的产品' : '暂无产品'}
          </Text>
          <Text className="block text-sm text-gray-400">
            {searchKeyword ? '请尝试其他关键词' : '点击上方按钮添加产品'}
          </Text>
        </View>
      ) : (
