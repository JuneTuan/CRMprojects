...
function WheelGame({ isSpinning, rotation }: any) {
  const prizes = [
    { name: '1元优惠券', color: '#FF6B6B', icon: '🎫', type: 'coupon' },
    { name: '2元红包', color: '#4ECDC4', icon: '🧧', type: 'redpacket' },
    { name: '3元优惠券', color: '#45B7D1', icon: '🎁', type: 'coupon' },
    { name: '5元红包', color: '#96CEB4', icon: '🧧', type: 'redpacket' },
    { name: '8元优惠券', color: '#FF4757', icon: '🎫', type: 'coupon' },
    { name: '10元红包', color: '#2ED573', icon: '🧧', type: 'redpacket' },
    { name: '15元优惠券', color: '#FFA502', icon: '🎁', type: 'coupon' },
    { name: '20元红包', color: '#5352ED', icon: '🧧', type: 'redpacket' },
    { name: '30元优惠券', color: '#FF6348', icon: '🎁', type: 'coupon' },
    { name: '50元红包', color: '#9B59B6', icon: '🧧', type: 'redpacket' },
  ]
  ...
  return (
    <View className="relative mb-6">
      ...
      <View className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1 z-30">
        <View className="w-0 h-0 border-l-[12px] border-r-[12px] border-t-[24px] border-l-transparent border-r-transparent border-t-red-600 drop-shadow-lg" />
      </View>

      {/* 转盘外圈 */}
      <View className="relative w-[320px] h-[320px] mx-auto rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 shadow-2xl p-1.5">
        {/* 转盘主体 */}
        <View
        ...
        >
            ...
            return (
              <View
                ...
                style={{
                  ...
                  backgroundColor: isEven ? prize.color : '#FFE4B5',
              ...
              >
                ...
                >
                  <Text className="block text-base mb-0.5">{prize.icon}</Text>
                  <Text className={`block text-[9px] font-bold leading-tight text-white`}>
                    {prize.name}
                  </Text>
                </View>
              </View>
          ...
          {/* 中心圆 */}
          <View className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[14%] h-[14%] bg-gradient-to-br from-red-600 to-orange-500 rounded-full shadow-lg border-[3px] border-white flex items-center justify-center z-20">
            <Text className="block text-[10px] font-bold text-white">抽奖</Text>
          </View>
        </View>
      </View>
    </View>
...
export default function IndexPage() {
      ...
      if (res.data.code === 200) {
        ...
        if (customers.length > 0) {
          ...
          if (createRes.data.code === 200) {
            setCustomer(createRes.data.data)
            fetchTodayCount(createRes.data.data.id)
          }
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (res.data.code === 200) {
        ...
        // 转盘游戏处理
          ...
          let prizeIndex = 5
          if (isWon && prize) {
            prizeIndex = prizes.findIndex(p => p.name === prize.name)
            if (prizeIndex === -1) {
  ...
  return (
    <View className="min-h-screen bg-gradient-to-b from-red-600 to-orange-500 p-6 pb-20">
      {/* 标题 */}
      <View className="text-center mb-8 pt-6">
        <Text className="block text-7xl mb-4">🧧</Text>
        <Text className="block text-3xl font-bold text-white">
          {activity ? `${activity.name}` : '春节幸运大转盘'}
        </Text>
        <Text className="block text-base text-red-100 mt-3">
      ...
      <View className="bg-white rounded-2xl p-6 mb-8 shadow-lg">
        <View className="flex justify-between items-center mb-4">
          <Text className="block text-base font-bold text-gray-800">👤 用户信息</Text>
          <Text className="block text-sm text-gray-400">{userInfo?.name || '管理员'}</Text>
        </View>
        {customer && (
          <View className="flex justify-between items-center">
            <View className="flex items-center">
              <Text className="block text-sm text-gray-500">积分:</Text>
              <Text className="block text-2xl font-bold text-orange-500 ml-2">{customer.points}</Text>
            </View>
            <View className="flex items-center">
              <Text className="block text-sm text-gray-500">剩余次数:</Text>
              <Text className="block text-2xl font-bold text-red-500 ml-2">{remainingCount}/3</Text>
            </View>
          </View>
        )}
        {!customer && (
          <Text className="block text-sm text-gray-400">请先在客户管理中创建客户信息</Text>
        )}
      </View>
      ...
      {activity?.gameType === 'wheel' && (
        <View className="text-center mb-8">
          <button
            className={`w-64 py-5 rounded-full font-bold text-xl text-white shadow-xl transform transition-transform ${
      ...
      {/* 抽奖规则 */}
      <View className="bg-white/20 backdrop-blur-sm rounded-xl p-6 mb-6">
        <Text className="block text-base font-bold text-white mb-4">📋 抽奖规则</Text>
        <View className="space-y-3">
          <View className="flex items-start">
            <Text className="text-white/90 text-sm mr-2">•</Text>
            <Text className="text-white/90 text-sm">每天可抽奖3次（免费）</Text>
          </View>
          {activity?.pointsEnabled && (
            <View className="flex items-start">
              <Text className="text-white/90 text-sm mr-2">•</Text>
              <Text className="text-white/90 text-sm">免费次数用完后可使用积分抽奖（{activity.pointsPerDraw}积分/次）</Text>
            </View>
          )}
          <View className="flex items-start">
            <Text className="text-white/90 text-sm mr-2">•</Text>
            <Text className="text-white/90 text-sm font-bold">8个奖品，中奖概率87.5%</Text>
          </View>
          <View className="flex items-start">
            <Text className="text-white/90 text-sm mr-2">•</Text>
            <Text className="text-white/90 text-sm">中奖后自动发放卡券到账户</Text>
          </View>
        </View>
      </View>
      ...
      {lastResult && (
        <View className="bg-white rounded-xl p-6 shadow-lg">
          <Text className="block text-base font-bold text-gray-800 mb-3">📌 最近抽奖结果</Text>
          <View className="flex items-center">
            <Text className="text-3xl mr-3">{lastResult.isWon ? '🎉' : '😊'}</Text>
            <Text className={`text-base font-semibold ${lastResult.isWon ? 'text-orange-500' : 'text-gray-500'}`}>
              {lastResult.result}
            </Text>
          </View>
        </View>
